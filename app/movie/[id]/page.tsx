...
export default async function MovieDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white p-4 sm:p-6 pt-8 max-w-7xl mx-auto selection:bg-sky-500 selection:text-white pb-20">
      
      {/* --- TOMBOL KEMBALI KE BERANDA (Desain Manis & Interaktif) --- */}
      <Link 
        href="/?type=movie" 
        className="group inline-flex items-center gap-2 text-slate-300 hover:text-sky-400 bg-slate-900/60 px-4 py-2 rounded-xl border border-slate-700/60 hover:border-sky-500/50 shadow-lg hover:shadow-sky-500/10 transition-all duration-300 font-semibold text-xs sm:text-sm mb-6 sm:mb-8 w-fit backdrop-blur-sm"
      >
        {/* Ikon Panah yang akan bergeser saat di-hover */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* 2. Komponen Iklan (Sekarang menempel manja tepat di bawah player) */}
